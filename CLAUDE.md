...
1. **credentials** — read the OAuth `accessToken`, **strictly read-only** (Pulse must NEVER refresh Claude Code's OAuth token or write to its credential stores — refresh tokens are single-use, so an uncoordinated refresh revokes the user's Claude Code login). Secret read: **both** stores — `~/.claude/.credentials.json` (JSON path `claudeAiOauth.accessToken`, fallback `accessToken`) and, on macOS, keychain item `Claude Code-credentials` (`security find-generic-password -s ... -w` / `SecItemCopyMatching`) — then **freshest wins**, see below.
   **CredentialCache contract (same state machine in all three ports — do not simplify):** the secret is cached in memory and every poll does only a *prompt-free fingerprint read* — the file's mtime (`"file:<mtime>"`) and the keychain item's `mdat` attribute (`"keychain:<mdat>"`; `security find-generic-password -s ...` **without `-w`** / `SecItemCopyMatching` with `kSecReturnAttributes` only — attribute reads never trigger keychain ACL prompts), **joined with `|` over every store that is present** (`combineFingerprints`); null = no credentials anywhere. The secret is re-read only when the fingerprint changes. On HTTP 401/403 the cache is invalidated and the request retried **once** iff the store already holds a different token; while the fingerprint still equals the rejected/denied one, polls throw without touching the secret (this gate is what prevents a keychain-prompt storm — a bare "re-read on 401" reintroduces it). Manual "Refresh Now" (`force`) bypasses only the rejection gate, never a healthy cache. Env var `CLAUDE_USAGE_TOKEN` bypasses everything (useful for testing / a `claude setup-token` token).
...
`expiresAt` (`pickFreshestToken`, pure and unit-tested in each port):

- Candidates are passed **file first, keychain last**, and ties go to the **last** candidate — the
  keychain, which is where Claude Code stores credentials on macOS.
- An unparseable or tokenless store is skipped, never fatal.
- The **fingerprint must combine every present store** (`combineFingerprints`), not just the first
  one. A file-first fingerprint is pinned to a dead file's mtime: it never changes, so the
  CredentialCache keeps serving the expired token and the keychain rotation is never noticed —
  "freshest wins" silently stops applying after the first read. Combining keeps the fingerprint
  read prompt-free (attributes only), so the prompt-storm guarantee above still holds.
- Consequence on macOS: with both stores present, the keychain secret is read whenever either store
  rotates, so each app asks for keychain permission (the user clicks *Always Allow*). That prompt is
  expected, not a bug.

This is a read-only selection: Pulse still never writes either store (see module 1).
...
- Network/transient error (incl. HTTP 429) with **no** prior value → neutral `··` placeholder in gray, the error text, and `formatRetryIn(delay)`; **no** login prompt. Telling the user to log in cannot fix a rate limit and sends them in circles. (`showTransient` / `applyTransient`; the tray icon draws `..` because its bitmap font has no MIDDLE DOT glyph.)
