...
## Shared architecture (same 5 modules in every implementation)
...
5. **tokens** — best-effort **local** token statistics, three rows per provider — `Tokens today: 1.2M in · 48K out · 9.8M cache`, `Tokens 7d: … · ▲ 12% vs prior 7d`, `Tokens 30d: … · — vs prior 30d` — in the tooltip / dropdown / tray menu only, never the status-bar line. **Only `Tokens today` is visible by default**; the 7d / 30d rows are revealed on demand: a hover submenu on the today row in `menubar/` (`makeTokensItem`) and `tray-go/` (a dedicated `tokenMenu` item per provider — never a positional detail slot, or every slot would carry a submenu arrow on Windows), and a `7d / 30d ▸` command link in the VSCode tooltip (`pulse.toggleTokenHistory`, an in-memory toggle that re-renders from cached state without polling; the MarkdownString trusts only that one command). All three split rows via `tokenMenuTitles` / the first `tokenRows` entry (`cache` = cache read + cache creation; the trend compares **total** tokens of the rolling window against the equally long prior window, and shows `—` until the ledger covers that prior window or when it is zero). Neither usage API returns token counts, so these come from the same transcripts `model` reads: every `~/.claude/projects/**/*.jsonl` **recursively** (the `<session>/subagents/agent-*.jsonl` files count too), bucketing `message.usage.{input_tokens, output_tokens, cache_read_input_tokens, cache_creation_input_tokens}` of `type == "assistant"` records by the **local date** of their `timestamp` (`src/tokens.ts`), and folding those buckets into a shared daily ledger (`src/tokenHistory.ts`, `tray-go/tokens_history.go`, `menubar/.../TokenHistory.swift`). Facts that constrain any change here:
   - Claude Code writes **one `assistant` line per content block** of a response, all sharing `message.id` with identical usage — count each `message.id` once or totals come out several times too high. Records without a `timestamp` are skipped (cannot be attributed to a window). `usage.iterations[]` is ignored.
   - **The ledger** lives in `$PULSE_HOME` (default `~/.pulse`), shared by all three apps: `token-history.json` = `{version: 1, since: {provider: date}, days: {date: {provider: {input, output, cacheRead, cacheCreate}}}}`, plus `cache/<provider>-files.json` = per-transcript `{mtime, size, days}`. Claude Code deletes transcripts after `cleanupPeriodDays` (default 30), so anything older than that exists **only** in the ledger — treat it as user data: never overwrite a file you could not parse (a corrupt ledger is renamed `token-history.json.corrupt-<ms>`), and never drop days you don't understand.
   - **CRITICAL: a ledger day is the per-field MAX ever observed, never a sum.** A day's total only grows while its transcripts are appended to and only shrinks when a transcript is deleted, so `max` preserves deleted files' contribution, cannot double count, and is also the merge rule when several apps write the file: every writer does read → max-merge (`mergeLedgers`) → atomic replace (temp + rename / `.atomic`). Adding instead of max-ing, or replacing without merging, silently corrupts everyone's history.
   - Only transcripts with mtime within the last 61 days (30d window + its prior 30d) are stat'ed and parsed; older days are served from the ledger. The per-file cache is keyed by path with `{mtime, size}`, so a normal poll re-reads only files that changed (a cold start parses ~61 days once, in the background — about a second for 270 MB in TS/Go, a few seconds in Swift). The cache is per provider root; a shared cache with a "remove entries not seen this walk" step made Claude and Codex evict each other every poll (fixed by this design — don't reintroduce it).
   - Codex (`tray-go/codex_tokens.go`, `menubar/.../CodexTokens.swift`; VSCode has no Codex): under `CODEX_HOME/sessions/**/*.jsonl`, prefer top-level `type == "token_usage_record"` (`payload.usage`, dedup by `payload.response_id`); only when a file has none, fall back to `event_msg` / `payload.type == "token_count"` → `payload.info.last_token_usage`, skipping `info == null` and consecutive identical repeats. **Never sum `total_token_usage`** — it is the session's running total. Codex `input_tokens` *includes* `cached_input_tokens`, so `input = input_tokens − cached_input_tokens`, `cacheRead = cached_input_tokens`, `cacheCreate = cache_write_input_tokens`.
   - Failure (no directory, unreadable file) omits the rows; it is never an error state. `pulse.showTokens` (VSCode, default true) disables the read; `Pulse --tokens` (tray) and `--once` / `--codex-once` (menubar) update the ledger and print the rows for verification, and `PULSE_HOME=/tmp/x` points them at a scratch ledger.
